...
def Newtrainees(request):
    if 'usernametm2' in request.session:
        ...
        teambt=team_batch.objects.all()
        team = create_team.objects.filter(~Q(team_status=1))
        mem1 = designation.objects.get(designation="trainee")
        des1 = designation.objects.get(designation='trainer')
        trainer = user_registration.objects.filter(designation_id=des1.id)
        memm = user_registration.objects.filter(designation_id=mem1).order_by('-id')
        return render(request, 'Newtrainees.html', {'mem': mem, 'memm': memm, 'des': des, 'dept': dept, 'team': team,'trainer':trainer,'teambt':teambt})
    else:
        return redirect('/')
...
def newtraineeesteam(request):
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            trainer=user_registration.objects.get(id=int(request.POST['trainer']))
            register = user_registration.objects.get(id=tid)
            register.course =course.objects.get(id=int(request.POST['cou']))
            print(request.POST.get('team'),'hai')
            teamename=team_batch.objects.get(id=int(request.POST.get('team')))
            register.team =create_team.objects.get(name=teamename.team_name,trainer_id=trainer.id)
            register.department =department.objects.get(id=int(request.POST['dept']))
            users =  previousTeam()
            users.teamname = create_team.objects.get(name=teamename.team_name,trainer_id=trainer.id)
...
def new_team_name(request):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm) .filter(fullname=usernametm1)
        team_name=team_batch.objects.filter(tb_status=0)
        
        des = designation.objects.get(designation='trainer')
        var2 = user_registration.objects.filter(designation_id=des.id)
        return render(request, 'new_team_name.html', {'mem': mem,'team_name':team_name,'var2':var2})
    else:
        return redirect('/')

    
def newteam_name_create(request):
    if 'usernametm2' in request.session:
        if request.session.has_key('usernametm'):
            usernametm = request.session['usernametm']
        if request.session.has_key('usernametm1'):
            ...
            return redirect('/')
        if request.method == 'POST':
            teamname = request.POST['tname']
            startdate=request.POST['teamstd']
            enddate=request.POST['teametd']
            team=team_batch()
            team.team_name=teamname
            team.tb_startdate=startdate
            team.tb_enddate=enddate
            team.tb_progress=0
            team.save()
...
def new_team1(request):
    if 'usernametm2' in request.session:
        ...
        team_name = team_batch.objects.all()
        return render(request, 'new_team1.html', {'mem': mem, 'var': var,'team_name':team_name})
    else:
        return redirect('/')
...
def trainee_new_team(request,tm_id):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm) .filter(fullname=usernametm1)
        print(tm_id)
        team1=create_team.objects.get(id=tm_id)

        var = create_team.objects.filter(trainer=team1.trainer,name=team1.name).filter(team_status=0).order_by('-id')
        
        pre=previousTeam.objects.filter(teamname=team1)
        des = designation.objects.get(designation='trainer')
        va = user_registration.objects.filter(designation_id=des.id)
       

        return render(request, 'trainee_new_team.html', {'mem': mem, 'var': var,'pre':pre,'va':va})
    else:
        return redirect('/')
...
def traineedetails(request,id):
    if 'usernametm2' in request.session:
        ...
        vars = user_registration.objects.get(id=id)
        tre = create_team.objects.get(id=vars.team.id)
        k=user_registration.objects.get(id=tre.trainerid)
        labels = []
        data = []
        queryset = user_registration.objects.filter(id=vars.id)
        for i in queryset:
...
def trainer_currentteam(request):
    if 'usernametrnr2' in request.session:
        ...
        tm = create_team.objects.filter(trainer_id=usernametrnr2).filter(team_status=0).order_by('-id')
...
def trainer_teamlistpage(request):
    if 'usernametrnr2' in request.session:
        ...
        tam = create_team.objects.filter(trainer_id=usernametrnr2,status=0).order_by('-id')
        des = designation.objects.get(designation='trainee')
        cut = user_registration.objects.filter(designation_id=des.id)
...
def trainer_taskgivenpage(request, id):
    if 'usernametrnr2' in request.session:
        ...
        if request.method == 'POST':
            ...
            tsk1=trainer_task.objects.all().order_by('-id').filter(id=usernametrnr2)
            return render(request, 'trainer_taskgiven.html', {'mem': mem,'mem1': mem1, 'tsk': tsk, 'z': z,'tsk1': tsk1,'time':time})
    else:
        return redirect('/')
...
def incompleted(request,id):
    if 'usernametrnr2' in request.session:
        ...
        if request.method=='POST':
            abc=trainer_task.objects.get(id=id)
            abc.correction_date=datetime.now()
            abc.verification=request.POST['verification']
            abc.correction_files=request.FILES['files']
            abc.task_status=2
            abc.save()
        return redirect('trainer_Task')
...
def trainee_task_list(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrns2)
        mem = trainer_task.objects.filter(user_id=usernametrns2).filter(Q(task_status=0)| Q(task_status=2)).order_by('-id')
...
def completed_team_trainees(request,tnt_id):
    if 'usernametm2' in request.session:
        if request.session.has_key('usernametm'):
            usernametm = request.session['usernametm']
        if request.session.has_key('usernametm1'):
            ...
            return redirect('/')
        mem = user_registration.objects.filter(designation_id=usernametm).filter(fullname=usernametm1)
        var = create_team.objects.filter(teambranch_id=tnt_id)
...
def Dm_datereport_data(request,retask_id,repj_id):
    if 'pm_id' in request.session:
        ...
        if 1 == retask_id:
            task_data=DataCollect.objects.filter(dc_date__gte=fromdate)
            print(task_data)
            rcount=0
            for i in task_data:
                if i.Project_name.dm_project_id.id == proj.id :
...
def DM_report_add(request):
    if 'pm_id' in request.session:
        ...
        return render(request, 'DigitalMarketing/DM__report_add.html',{'mem': mem,'proj':proj})
    else:
        return redirect('/')
